...
class DirectorTests(unittest.TestCase):
 ...
 def test_execute_rechecks_stale_targets(self):
  plan=Plan("x","",1,"",[Step("focus","0xgone")],executable=True); self.store.save_plan({**plan.to_dict(),"query":"","created_at":0,"snapshot":{}})
 ...
 def test_arrangement_never_pixel_moves_tiled_windows(self):
  plan=Plan("x","",1,"",[Step("arrange","0xaaa",layout="tile")],executable=True); self.store.save_plan({**plan.to_dict(),"query":"","created_at":0,"snapshot":{}})
 ...
 def test_arrange_moves_and_switches_to_selected_workspace(self):
  plan=Plan("x","",1,"",[Step("arrange","0xaaa",2,"tile")],executable=True); self.store.save_plan({**plan.to_dict(),"query":"","created_at":0,"snapshot":{}})
 ...
 def test_isolate_switches_to_destination(self):
  plan=Plan("x","",1,"",[Step("isolate","0xaaa",2)],executable=True); self.store.save_plan({**plan.to_dict(),"query":"","created_at":0,"snapshot":{}})
...
class DesktopTests(unittest.TestCase):
 ...
 def test_launch_exact_argv(self):
  runner=Mock(return_value=Result()); launch("safe",{"safe":{"id":"safe"}},runner); runner.assert_called_once_with(["gtk-launch","safe"],capture_output=True,text=True,check=False)
