...
usage() {
  cat <<'EOF'
Usage: ./install.sh [--bindings] [--voice-bindings]
...
[[ "$repo_dir" == "$plugin_dir" ]] || {
  ...
  echo "Install it first with: omarchy plugin add https://github.com/ieltxu/omarchy-director.git --enable" >&2
  exit 2
}

for command_name in python3 jq omarchy omarchy-shell; do
...
desktop_file="$apps_dir/ieltxu-director.desktop"
...
if [[ "$install_bindings" == true ]]; then
  [[ -f "$bindings_file" ]] || { echo "Hyprland bindings file not found: $bindings_file" >&2; exit 1; }
